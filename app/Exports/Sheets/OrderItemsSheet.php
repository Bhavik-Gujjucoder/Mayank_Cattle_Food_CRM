    ...
    public function collection(): Collection
    {
        return $this->items->values()->map(function (RawMaterialOrderItem $item, int $index) {
            ...
            $row = array_merge($row, [
                ...
                number_format(\App\Services\RawMaterialCacheService::itemTaxAmount($item), 2),
                number_format((float) $item->other_expense, 2),
                number_format((float) $item->tds_amount, 2),
                number_format(\App\Services\RawMaterialCacheService::itemTotalAmount($item), 2),
                number_format(\App\Services\RawMaterialCacheService::itemPendingAmount($item), 2),
